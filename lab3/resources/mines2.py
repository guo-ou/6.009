...
def new_game(num_rows, num_cols, bombs):
    ...
    for br, bc in bombs:
        for nr, nc in neighbors([num_rows, num_cols], br, bc):
            if board[nr][nc] != '.':
                board[nr][nc] += 1
    return {"dimensions": [num_rows, num_cols], "board" : board, "mask" : board, "state": "ongoing"} #NOTE: mask:board
...
def dig(game, row, col):
    def do_dig(game, row, col):
        ...
        if not game["mask"][row][col] and type(game["board"][row][col]) == int:
            game["board"][row][col] = True #NOTE:Just uncovers all but the bombs on the start of dig

            if game["board"][row][col] != 0:
                return #NOTE: fails dig_count
            else:
                coord1 = [row - 1, col - 1]
                coord2 = [row - 1, col]
                coord3 = [row - 1, col + 1]
    ...
    def revealed_count(game):
        ...
        for row in game["mask"]:
            revealed += len([x for x in row if x == True]) #NOTE: doesnt take into account which were true to begin with
        return revealed
    ...
    do_dig(game,row,col)
    dug = revealed_count(game) #NOTE: should subtract squares_revealed_initial
    ...
    for r in range(len(game["board"])):
        for c in range(len(game["board"][r])):
            if check_correct(game, r, c):
                ...
                no_victory = True
    if not no_victory: #NOTE: should be the opposite
        game['state'] = victory
        return dug
