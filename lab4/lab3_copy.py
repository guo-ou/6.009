...
def new_game(num_rows, num_cols, bombs):
    ...
    depth = 0
    def grab_obj_from_coord(array, coord, depth, obj=None):
        if depth == len(coord) -1:
            ...
            return grab_obj_from_coord(array, coord, depth + 1, obj)

    def get_neighbors(dimensions, coord, working_coord=[], coord_list=set()):
        coord = list(coord)
        ...
        if len(coord) == 0:
            ...
            for xi in range(-1,2):
                if coord[0] + xi < 0 or coord[0] + xi > dimensions[0] -1:
                    continue

                working_coord.append(coord[0] + xi)

                get_neighbors(dimensions[1:], coord[1:], working_coord, coord_list)
                working_coord.pop()
    ...
    depth = 0
    def recursiveArray(dimensions, depth, mask=""):
        val = 0
        if mask == "mask":
            val = False
        ...
        #Recursive Case:
            ...
            curdim = dimensions[depth]
            for i in range(curdim):
                array.append(recursiveArray(dimensions, depth + 1, mask))

            return array

    board = recursiveArray([5,5,5], depth)
    mask = recursiveArray([5,5,5], depth, "mask")
    dimensions = [5,5,5]

    # board[1][1][1] = "ADASD"

    def build_bomb_count(array, bombs):
        for bomb in bombs:
            for coord in get_neighbors(dimensions, bomb):
                if coord in bombs:
                    continue
                grab_obj_from_coord(array, coord, 0, "add")
            val = grab_obj_from_coord(array, bomb, 0,".")
